	...
	public class TaskQueue extends EventDispatcher {
		...
		public function failure(event : Event) : void {
			...
			if (event is ErrorEvent) {
				dispatchEvent(new ErrorEvent(ErrorEvent.ERROR, false, false, "Task Queue failed at step " + taskIndex + ": " + ErrorEvent(event).text));
			} else if (event is PlayerEvent && (event as PlayerEvent).message) {
				dispatchEvent(new ErrorEvent(ErrorEvent.ERROR, false, false, "Task Queue failed at step " + taskIndex + ": " + PlayerEvent(event).message));
			} else {
				dispatchEvent(new ErrorEvent(ErrorEvent.ERROR, false, false, "Task Queue failed at step " + taskIndex + ": " + event.toString()));
			}
