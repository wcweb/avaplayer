package com.longtailvideo.jwplayer.view.components {
	...
	public class ControlbarComponent extends CoreComponent implements IControlbarComponent {
		...
		public function ControlbarComponent(player : IPlayer) {
			...
			_dividers = [];
			_dividerElements = {'divider':setupDivider('divider')};
			setupBackground();
			setupDefaultButtons();
			addEventListeners();
		...
		private function addSlider(name : String, event : String, callback : Function, margin : Number = 0) : void {
			try {
				var slider : Slider = new Slider(getSkinElement(name + "SliderRail"), getSkinElement(name + "SliderBuffer"), getSkinElement(name + "SliderProgress"), getSkinElement(name + "SliderThumb"), getSkinElement(name + "SliderCapLeft"), getSkinElement(name + "SliderCapRight"));
