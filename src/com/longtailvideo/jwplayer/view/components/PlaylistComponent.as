package com.longtailvideo.jwplayer.view.components {
	...
	public class PlaylistComponent extends CoreComponent implements IPlaylistComponent {
		...
		private function buildSliderElement(name : String, skinElementName : String, width : Number = 0, height : Number = 0) : Sprite {
			...
			// This is not possible if the element was created and named from an FLA
		...
		private function setContents(idx : Number) : void {
			...
			try {
				...
				if (fontColor) {
					if (description) {
						description.textColor = fontColor.color;
					}
					if (title) {
						title.textColor = fontColor.color;
					}
					if (author) {
						author.textColor = fontColor.color;
					}
					if (tags) {
						tags.textColor = fontColor.color;
					}
				} else if (front) {
					if (description) {
					...
					if (author) {
						author.textColor = front.color;
					}
					if (tags) {
						tags.textColor = front.color;
					}
				}
			} catch (e : Error) {
			}
