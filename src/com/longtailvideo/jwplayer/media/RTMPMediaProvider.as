	...
	public class RTMPMediaProvider extends MediaProvider {
		...
		private function doDVRInfoCallback(info : Object) : void {
			...
			if (info.code == "NetStream.DVRStreamInfo.Success") {
				...
				if (_subscribeCount > 3) {
					...
					stop();
					sendMediaEvent(MediaEvent.JWPLAYER_MEDIA_ERROR, {message:"Subscribing to the dvr stream timed out."});
				} else {
		...
		private function doSubscribe() : void {
			...
			if (_subscribeCount > 3) {
				clearInterval(_subscribeInterval);
				stop();
				sendMediaEvent(MediaEvent.JWPLAYER_MEDIA_ERROR, {message:"Subscribing to the live stream timed out."});
		...
		private function getStreamInfo() : void {
			...
			try {
				...
				if (len > 5 && state == PlayerState.PLAYING) {
					bwd = Math.round((_streamInfo[len - 1].bwd + _streamInfo[len - 2].bwd + _streamInfo[len - 3].bwd + _streamInfo[len - 4].bwd + + _streamInfo[len - 5].bwd) / 5);
					config.bandwidth = bwd;
					Configger.saveCookie('bandwidth', bwd);
					// Don't trust framedrops when player buffered during last samplings.
					if (_streamInfo[len - 2].stt == PlayerState.BUFFERING || _streamInfo[len - 3].stt == PlayerState.BUFFERING) {
						drf = 0;
					} else {
		...
		private function swap(newLevel : Number) : void {
			if (_transitionLevel == -1 && (newLevel < item.currentLevel || _stream.bufferLength < _stream.bufferTime * 1.5 || item.levels[item.currentLevel].blacklisted)) {
