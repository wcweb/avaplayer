package com.longtailvideo.jwplayer.model {
	...
	public dynamic class PlaylistItem {
			...
			for (var itm:String in obj) {
				if (itm == "levels") {
					...
					for each (var level:Object in levels) {
						if (level['file']) {
							var newLevel : PlaylistItemLevel = new PlaylistItemLevel(level['file'], Number(level['bitrate']), Number(level['width']), level['streamer']);
		...
		public function toString() : String {
			return "[PlaylistItem" + (this.file ? " file=" + this.file : "") + (this.streamer ? " streamer=" + this.streamer : "") + (this.provider ? " provider=" + this.provider : "") + (this.levels.length ? " level=" + this.currentLevel.toString() : "") + "]";
		}
