		...
		public static function parse(dat : XML) : Array {
			...
			if (switchOrVideo.localName().toLowerCase() == 'switch') {
				for each (var i:XML in switchOrVideo.children()) {
					var level : PlaylistItemLevel = new PlaylistItemLevel(Strings.xmlAttribute(i, 'src'), Number(Strings.xmlAttribute(i, 'system-bitrate')) / 1000, Number(Strings.xmlAttribute(i, 'width')), streamer);
					array.push(level);
				}
			} else {
				array.push({file:Strings.xmlAttribute(switchOrVideo, 'src'), streamer:streamer});
			}
			return array;
		};
	}
