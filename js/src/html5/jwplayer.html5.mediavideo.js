			...
			if ((event.target.error && event.target.tagName.toLowerCase() == "video") ||
				...
				switch (element.code) {
					...
					case element.MEDIA_ERR_NETWORK:
						message = "网络问题导致视频下载半路失败: ";
						break;
					case element.MEDIA_ERR_DECODE:
						message = "因为突发事件，无法编译视频，换个浏览器试试: ";
						break;
					case element.MEDIA_ERR_SRC_NOT_SUPPORTED:
						message = "视频无法被加载，可能因为网络，服务器或者视频格式不支持: ";
						break;
					default:
						message = "连我都不知道发生什么事了: ";
						break;
				}
				...
				if (_utils.userAgentMatch(/firefox/i)) {
					...
					_utils.log("你现在在用firefox浏览器吗？ 哎呀，不行啊.");
					_stop(false);
					return;
				} else {
					message = "嗯， 你现在是在用先进的浏览器吗？我只能肯定，你不是在用firefox，: ";
				}
			} else {
				_utils.log("哎呀，连我自己都不知道发生什么事了。 生活还是要继续的……");
				return;
			}
			_stop(false);
			message += _joinFiles();
